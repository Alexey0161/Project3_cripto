...
Примечание: Сборка образа при первом запуске может занять 2-3 минуты, так как Docker будет устанавливать зависимости из requirements.txt автоматически.
...
Система использует динамическое определение путей (os.path), поэтому база данных будет корректно создана в dags/bd_sqlite/ независимо от того, запускаете вы проект из корня или из папки скрипта.
