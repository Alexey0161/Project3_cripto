Данный репозиторий содержит итоговую сборку проектов 3 и 4
...
## 🌌  Управление оркестрацией в Airflow

Интерфейс Airflow доступен в браузере по адресу: [http://localhost:8080](http://localhost:8080) (или порт 8081, в зависимости от настроек).

* **Логин по умолчанию:** `admin`
* **Как получить актуальный пароль:** Поскольку Airflow при каждом чистом старте генерирует случайный пароль, выполните в терминале команду для чтения файла из запущенного контейнера:
  ```bash
  docker exec -it project3_cryptoscraper-airflow-master-1 cat standalone_admin_password.txt
...
3.1. **Клонируйте репозиторий:**
 ```bash
   git clone [https://github.com/Alexey0161/Project3_cripto.git](https://github.com/Alexey0161/Project3_cripto.git)
   cd Project3_cripto
...
Тестирование API через интерактивную документацию (Swagger UI):

После запуска сервера откройте браузер и перейдите по адресу: http://127.0.0.1:8008/docs

На открывшейся странице найдите синий блок GET /prices и кликните на него.

В правом углу развернувшегося окна нажмите кнопку "Try it out" (Начать тестирование).

В появившемся поле ticker введите заглавными буквами имя монеты, например: BTC.

Опуститесь чуть ниже и нажмите большую синюю кнопку "Execute" (Выполнить).

В разделе "Server response" (Ответ сервера) ниже отобразится актуальная информация по крипте в формате JSON.
