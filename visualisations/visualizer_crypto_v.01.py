...
# --- АРТЕФАКТ №4: Индивидуальные графики  ---
    ...
    plt.title(
        "Динамика цен криптовалют (Очищенные данные)", fontsize=16, fontweight="bold"
    )
    plt.xlabel("Дата и время замера", fontsize=12)
    plt.ylabel("Цена (USD) - Log scale", fontsize=12)
    ...
    report1 = (
        ...
        / df.groupby("coin_name")["price"].agg("count")
        * 2
    )
